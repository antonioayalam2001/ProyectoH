...
<table class="table table-striped table-hover">
    <tr class="bg-primary text-center text-white">
        <th>IdCategoria</th>
        <th>MombreCategoria</th>
        <th>DescricionCategoria</th>
        <th>Eliminar</th>
        <th>Actualizar</th>
    </tr>
    ...
    <c:forEach items="${listado}" var="dto">
        ...
        <td class="align-middle "><a href="./CategoriasServlet?accion=ver&id=<c:out value="${dto.idCategoria}"/>" class="btnP"><c:out value="${dto.idCategoria}"/></a> </td>
        <td class="align-middle "><c:out value="${dto.nombreCategoria}"/>
        <td class="align-middle "><c:out value="${dto.descripcionCategoria}"/></td>
        <td class="align-middle "><a href="./CategoriasServlet?accion=eliminar&id=<c:out value="${dto.idCategoria}"/>" class="btn btn-danger p-2 rounded-circle"><i class='bx bxs-trash'></i></a></td>
        <td class="align-middle "><a href="./CategoriasServlet?accion=actualizar&id=<c:out value="${dto.idCategoria}"/>" class="btn btn-primary p-2 text-white rounded-circle"><i class='bx bx-edit bx-sm'></i></a></td>
    </tr>
        <%--Making things with JSTL--%>
<%--        <c:set value="${dto.entidad.idCategoria}" var="num"></c:set>--%>
<%--        <c:choose>--%>
<%--            <c:when test="${num == 1}">--%>
<%--                <h1><c:out value="${num} Este es el primer elemento que se encontro"></c:out></h1>--%>
<%--            </c:when>--%>
<%--            <c:otherwise>--%>
<%--                <h1>El elemento no fue el primero fue el numero <c:out value="${num}"></c:out></h1>--%>
<%--            </c:otherwise>--%>
<%--        </c:choose>--%>
...
<%--<h1><c:out value="${frases}"></c:out></h1>--%>
<%--<h1><c:out value="${frases[0]}"></c:out></h1>--%>
<%--<h1><c:out value="${frases[1]}"></c:out></h1>--%>


<%--Monstrando nuestra nueva variable--%>
<%--<h1>Recibiendo el dato del servidor <c:out value="${numero}"></c:out></h1>--%>

<%--<h2>Importanto un valor desde una link de otro lugar--%>
<%--    <c:import var="data" url="http://www.javatpoint.com"/>--%>
<%--    <c:out value="${data}"/>--%>
<%--</h2>--%>

<%--&lt;%&ndash;Declarando una variable &ndash;%&gt;--%>
<%--<c:set var="income"  value="${4000*4}"/>--%>
<%--<h2><c:out value="${income}"/></h2>--%>

<%--<c:set var="cadena" scope="session" value="Declare cadena"/>--%>
<%--<h2><c:out value="${cadena}"/></h2>--%>


<%--&lt;%&ndash;Ahora eliminamos el valor de la variable&ndash;%&gt;--%>
<%--<c:remove var="income"/>--%>
<%--<p>After Remove Value is: <c:out value="${income}"/></p>--%>

<%--&lt;%&ndash;Realizando el manejo de algun error&ndash;%&gt;--%>
...
<%--&lt;%&ndash;Realizando la prueba de un if&ndash;%&gt;--%>
<%--<c:if test="${numero <20}" >--%>
<%--    <p>El valor no es 20</p>--%>
<%--</c:if>--%>
<%--<c:choose>--%>
<%--    <c:when test="${numero <= 1000}">--%>
<%--        numero is not good.--%>
<%--    </c:when>--%>
<%--    <c:when test="${numero > 10000}">--%>
<%--        numero is very good.--%>
<%--    </c:when>--%>
<%--    <c:otherwise>--%>
<%--        numero is undetermined...--%>
<%--    </c:otherwise>--%>
<%--</c:choose>--%>
<%--<c:choose>--%>
<%--    <c:when test="${fn:contains(frases[0],'Hola')}">--%>
<%--        Encontre la cadena hola--%>
<%--    </c:when>--%>
<%--    <c:when test="${fn:contains(frases[0],'Adios')}">--%>
<%--        Encontre adios--%>
<%--    </c:when>--%>
<%--    <c:otherwise>--%>
<%--        No encontre nada--%>
